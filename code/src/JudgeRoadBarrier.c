...
int JudgeRoadBarrier(void)
{
    if(Barrier_flag == 0)
    {
      ...
      {      
        if(RealWidth>=35&&RealWidth<=55&&MidLine[ControlRow]>35&&MidLine[ControlRow]<116)//RealWidth<=55MidLine[ControlRow]>60&&MidLine[ControlRow]<90
        {
          if(Right_Slope[ControlRow]>-0.8 && Right_Slope[ControlRow]<=0 && Left_Slope[ControlRow]<0.8 && Left_Slope[ControlRow]>=0)
          {
            ...
            if(BarrierLeftSlopeVariance<0.2)
            {
              ...
              else {discount=0;BarrierError = 4;return 0;}
            }
            else {discount=0;BarrierError = 3;return 0;}
          }
          else {discount=0;BarrierError = 2;return 0;}
        }
        else {discount=0;BarrierError = 1;return 0;}
      }
      else {discount=0;BarrierError = 3;return 0;}
  }  
  return 0;
