...
{
	int i;
	for (i = ROW - 1; i > 0; i--) { videoData[i][76] = ~videoData[i][76]; }
}
...
void GetRealMidLine(void)
{
	...
	if (Circle_flag == 2) {
		...
		if (R_Kgin[CircleNumber] > 10000)
			R_Kgin[CircleNumber] = -R_Kgin[CircleNumber] + 10000;
		...
		if (Circle_Direction == 1) {
			...
			RealMidLine1[ControlRow] = RealMidLine1[ControlRow] - CircledtL6 - Cirlce_6_add;
		}	 //-20.0
