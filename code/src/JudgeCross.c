...
// 用于保存左右臂远近端点
int leftNearFound[1][2], leftRemoteFound[1][2], rightNearFound[1][2], rightRemoteFound[1][2];
// 左右臂存在标志位
 ...
 */
static int8 FindCrossArm(int coordinate1[1][2], int coordinate2[1][2], int start_X, int start_Y, int dir)
{
	int i		 = 0;
	...
	for (i = start_y; i > checkRow; i--) {
		sum = 0;
		if (videoData[i][start_x] == 1 && videoData[i - 1][start_x] == 1 && videoData[i - 2][start_x] == 1) {
			near = i;
			for (i = i - 3; i > checkRow; i--) {
				if (start_x >= LeftLine[i] && start_x <= RightLine[i]) {
					sum++;
				}
				if (videoData[i][start_x] == 0 && videoData[i - 1][start_x] == 0 && videoData[i - 2][start_x] == 0) {
					remote = i + 1;
					if (dir == 1)
						myCrossLengthLeft = GetLength(start_x, near, start_x, remote);
					else if (dir == 2)
						myCrossLengthRight = GetLength(start_x, near, start_x, remote);
					if (sum >= 2 && AssertLength(start_x, near, start_x, remote, 20, 65, &ArmLength)) {
...
void  GetCrossInterSectionPoint(void)
{
	...
	for (i = ROW - 10; i > CrossCheckRow; i--) {
		if (Left_Slope[i - 2] < 0 && Left_Slope[i - 2] - Left_Slope[i + 2] < -0.8 && Left_Slope[i - 1] - Left_Slope[i + 1] < -0.3) {
			int LeftMin = LeftLine[i + 2];
			for (j = i; j > i - 5; j--) {
				if (videoData[j][LeftLine[i]] == 0 && videoData[j - 1][LeftLine[i]] == 0) {
					break;
				}
			}
			if (j > i - 5)
			...
			if (LeftMin - LeftLine[i + 2] > -5)
				continue;
			LeftFound = i - 3;
			for (i = LeftFound; i < ROW - 6; i++) {
				if (LeftLine[i] >= LeftLine[i + 2] && LeftLine[i + 2] >= LeftLine[i + 4]) {
					LeftFound = i + 2;
					break;
				}
			}
	...
	for (i = ROW - 10; i > CrossCheckRow; i--) {
		if (Right_Slope[i - 2] > 0 && Right_Slope[i - 2] - Right_Slope[i + 2] > 0.8
			&& Right_Slope[i - 1] - Right_Slope[i + 1] > 0.3) {
			int RightMax = RightLine[i + 2];
			for (j = i; j > i - 5; j--) {
				if (videoData[j][RightLine[i]] == 0 && videoData[j - 1][RightLine[i]] == 0) {
					break;
				}
			}
			if (j > i - 5)
			...
			if (RightMax - RightLine[i + 2] < 5)
				continue;
			RightFound = i - 3;
			for (i = RightFound; i < ROW - 6; i++) {
				if (RightLine[i] <= RightLine[i + 2] && RightLine[i + 2] <= RightLine[i + 4]) {
					RightFound = i + 2;
					break;
				}
			}
...
int Judge_Cross(void)
{
	...
	LeftCrossFlag  = FindCrossArm(leftNearFound, leftRemoteFound, LeftStart[0][0], LeftStart[0][1], 1);		   // 识别左臂
	RightCrossFlag = FindCrossArm(rightNearFound, rightRemoteFound, RightStart[0][0], RightStart[0][1], 2);	   // 识别右臂
	...
	if (LeftCrossFlag == 1 && RightCrossFlag == 1) {
		...
		for (i = GetMax(leftNearFound[0][1], rightNearFound[0][1]); i < ROW - 2; i++) {
			if (AssertLength(LeftLine[i], i, RightLine[i], i, 30, 48, &CrossWidth))	   // 判断赛道宽度是否接近正常赛道 45可能要改
			{
				InflexionNear = i;
				break;
			}
		...
		for (i = GetMin(leftRemoteFound[0][1], rightRemoteFound[0][1]); i > RealEndRow + 5; i--) {
			if (AssertLength(LeftLine[i], i, RightLine[i], i, 30, 48, &CrossWidth))	   // 判断赛道宽度是否接近正常赛道
			{
				InflexionFar = i;
				break;
			}
...
void CrossDeal(void)
{
	...
	if (ImageFlag) {
		...
		for (i = InflexionNear; i >= InflexionFar; i--) {
			MidLine[i] = (MidLine[InflexionNear + RowStep] * (i - InflexionFar + RowStep)
						  + MidLine[InflexionFar - RowStep] * (InflexionNear + RowStep - i))
						 / (InflexionNear - InflexionFar + 2 * RowStep);
...
uint8 Judge_Curve_Cross(void)
{
	...
	if (Leftwhite > Rightwhite) {
		...
		for (i = ROW - 3; i > endline; i--) {
			if (i <= 2)
				return 0;	 // 防止i-2越界导致程序卡住
			if ((Right_Slope[i] < 0 && Right_Slope[i + 1] < 0 && Right_Slope[i + 2] < 0 && Right_Slope[i - 1] > 0
				 && Right_Slope[i - 2] > 0)
				|| (Right_Slope[i] == 0 && Right_Slope[i + 1] <= 0 && Right_Slope[i - 1] > 0 && Right_Slope[i - 2] > 0)
				|| (i < 93 && Right_Slope[i] == 0 && Right_Slope[i + 1] <= 0 && Right_Slope[i + 2] <= 0 && Right_Slope[i + 3] <= 0
					&& Right_Slope[i + 4] <= 0 && Right_Slope[i + 5] <= 0 && Right_Slope[i + 6] < 0 && Right_Slope[i - 1] > 0
					&& Right_Slope[i - 2] > 0)) {
				Cross_start	 = i + 1;
				...
				for (j = i; j > EndRow + 1; j--) {
					if (j < 3)
						return 0;
					if (RightLine[j] - RightLine[j - 1] > 10 && RightLine[j] - RightLine[j - 2] > 10
						&& RightLine[j - 2] < RightLine[Cross_start])	 // Left_Slope[j]<0 && Left_Slope[j+1]<0 &&
																		 // Left_Slope[j-1]>0 && Left_Slope[j-2]>0
					{
						Cross_end  = j - 1;
		...
		if (AssertLength(RightLine[Cross_start], Cross_start, RightLine[Cross_end], Cross_end, 33, 49, &Curve_ArmWidth)) {
			if (Cross_start + 2 + 20 > ROW - 3)
				temprow1 = ROW - 3;
			else
				temprow1 = Cross_start + 2 + 20;
			if (Cross_end - 2 - 10 < EndRow + 5)
				temprow2 = EndRow + 5;
			else
				temprow2 = Cross_end - 2 - 10;
		...
		for (i = ROW - 3; i > endline; i--) {
			if (i <= 2)
				return 0;	 // 防止i-2越界导致程序卡住
			if ((Left_Slope[i] > 0 && Left_Slope[i + 1] > 0 && Left_Slope[i + 2] > 0 && Left_Slope[i - 1] < 0
				 && Left_Slope[i - 2] < 0)
				|| (Left_Slope[i] == 0 && Left_Slope[i + 1] >= 0 && Left_Slope[i + 2] > 0 && Left_Slope[i - 1] < 0
					&& Left_Slope[i - 2] < 0)
				|| (i < 93 && Left_Slope[i] == 0 && Left_Slope[i + 1] >= 0 && Left_Slope[i + 2] >= 0 && Left_Slope[i + 3] >= 0
					&& Left_Slope[i + 4] >= 0 && Left_Slope[i + 5] >= 0 && Left_Slope[i + 6] > 0 && Left_Slope[i - 1] < 0
					&& Left_Slope[i - 2] < 0)) {
				Cross_start	 = i + 1;
				Cross_start1 = Cross_start;
				for (j = i; j > EndRow + 1; j--) {
					if (j < 3)
						return 0;
					if (LeftLine[j - 1] - LeftLine[j] > 10 && LeftLine[j - 2] - LeftLine[j] > 10
						&& LeftLine[j - 2] > LeftLine[Cross_start])	   // Left_Slope[j]<0 && Left_Slope[j+1]<0 && Left_Slope[j-1]>0
																	   // && Left_Slope[j-2]>0
					{
						Cross_end  = j - 1;
						Cross_end1 = Cross_end;
...
void Curve_Cross_Deal(void)
{
	...
	for (i = Cross_start1; i >= Cross_end1; i--) {
		MidLine[i] = MidLine[Cross_end1 - RowStep]
					 + (i - Cross_end1 + RowStep) * (MidLine[Cross_start1 + RowStep] - MidLine[Cross_end1 - RowStep])
						   / (Cross_start1 - Cross_end1 + 2 * RowStep);
		Error[i] = MidLine[i] - COLUMN / 2;
	}
