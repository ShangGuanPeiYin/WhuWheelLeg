...
int SuperiorJudge(RoadConditions roadJudge)
{
  switch(roadJudge)
  {
    ...
    case LOOP:
    {
      ...
      {
        RoadFlag[LOOP]=1;
        if(JudgeRound_flag == 1) Roundabout_Deal();
        return 1;
      }
      break;
    }
...
void Image_To_Warp(void)
{
  ...
  {   
    if(BarrierDir==1) RealWarp =(-1)*Ba_RealWarp1/10000.0;//-0.5073
    else RealWarp =Ba_RealWarp1/10000.0;
  }
  else
  {
    RealWarp=(RealMidLine1[ControlRow]*0.5f
              +RealMidLine1[ControlRow+RowStep]*0.15f
