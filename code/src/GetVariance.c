...
double GetVariance(int row0,int row1)
{
  ...
  for(i=row0;i>row1;i--)
  {
    Variance+=(MidLine_Slope[i]-MidLine_Slope_Avg)*(MidLine_Slope[i]-MidLine_Slope_Avg);
  }
...
double GetVariance1(int row0,int row1)
{
  ...
  for(i=row0;i>row1;i-=2)
  {
    Variance+=(RealMidSlope[i]-MidLine_Slope_Avg)*(RealMidSlope[i]-MidLine_Slope_Avg);
  }
...
double GetLeftVariance(int row0,int row1)
{
  ...
  for(i=row0;i>row1;i-=2)
  {
   Variance+=(Left_Slope[i]-LeftLine_Slope_Avg)*(Left_Slope[i]-LeftLine_Slope_Avg);
  }
  Variance = Variance/count;
  return Variance;
}
