...
int abs_int(int x) { return x = x > 0 ? x : -x; }

float abs_float(float x) { return x = x > 0 ? x : -x; }
...
float LimitMax(float a, float max) { return a > max ? max : a; }

float LimitMin(float a, float min) { return a < min ? min : a; }
