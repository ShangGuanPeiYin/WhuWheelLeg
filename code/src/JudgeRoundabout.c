...
void Circle_ControlRow_Set(void)
{
	switch (CircleNumber) {
		case 0:
			Circle_ControlRow[0] = 0;
			break;
		case 1:
			Circle_ControlRow[1] = CircleControlRow1;
			break;
		case 2:
			Circle_ControlRow[2] = CircleControlRow2;
			break;
		case 3:
			Circle_ControlRow[3] = CircleControlRow3;
			break;
		case 4:
			Circle_ControlRow[4] = CircleControlRow4;
			break;
		default:
			break;
	}
...
void Roundabout_Deal(void)
{
	...
	if (Circle_flag == 1) {
		...
		if (Circle_Direction == 1) {
			...
			if (down_prerow) {
				...
				if (ImageFlag) {
					// 显示补线
					for (i = (uint8) RightFeatureRow; i <= (uint8) down_prerow; i++) {
						Revise_RightLine[i]
							= Revise_road(RightLine[down_prerow], down_prerow, RightLine[RightFeatureRow], RightFeatureRow, i);
					}
			...
			if (down_prerow) {
				...
				if (ImageFlag) {
					// 显示补线
					for (i = (uint8) LeftFeatureRow; i <= (uint8) down_prerow; i++) {
						Revise_LeftLine[i]
							= Revise_road(LeftLine[down_prerow], down_prerow, LeftLine[LeftFeatureRow], LeftFeatureRow, i);
					}
	...
	if (Circle_flag == 8) {
		// 对控制行的限制
		if (Circle_ControlRow[CircleNumber] <= EndRow + 5 && EndRow + 5 < ROW)
			ControlRow = EndRow + 5;
		else if (Circle_ControlRow[CircleNumber] <= EndRow + 5 && EndRow + 5 > ROW - 1)
			ControlRow = ROW - 1;
		else {
			ControlRow = Circle_ControlRow[CircleNumber];
		}
		...
		if (Circle_Direction == 1) {
			for (j = ROW - 5; j > EndRow + 5; j++) {
				if ((RightLine[j + 1] - RightLine[j] > 4 && RightLine[j + 2] - RightLine[j] > 4
					 && RightLine[j + 3] - RightLine[j] > 4 && RightLine[j + 1] - RightLine[j - 1] > 4
					 && RightLine[j + 2] - RightLine[j - 1] > 4 && RightLine[j + 3] - RightLine[j - 1] > 4
					 && RightLine[j + 1] - RightLine[j - 2] > 4 && RightLine[j + 2] - RightLine[j - 2] > 4
					 && RightLine[j + 3] - RightLine[j - 2] > 4 && RightLine[j + 1] == 151 && RightLine[j + 2] == 151
					 && RightLine[j + 3] == 151)
					|| (RightLine[j + 2] - RightLine[j] > 4 && RightLine[j + 3] - RightLine[j] > 4
						&& RightLine[j + 4] - RightLine[j] > 4 && RightLine[j + 1] - RightLine[j - 1] > 4
						&& RightLine[j + 2] - RightLine[j - 1] > 4 && RightLine[j + 3] - RightLine[j - 1] > 4
						&& RightLine[j + 1] - RightLine[j - 2] > 4 && RightLine[j + 2] - RightLine[j - 2] > 4
						&& RightLine[j + 3] - RightLine[j - 2] > 4 && RightLine[j + 2] == 151 && RightLine[j + 3] == 151
						&& RightLine[j + 4] == 151)) {
					Circle_flag = 9;
			...
			for (j = ROW - 5; j > EndRow + 5; j--) {
				if ((LeftLine[j] - LeftLine[j + 1] > 4 && LeftLine[j] - LeftLine[j + 2] > 4 && LeftLine[j] - LeftLine[j + 3] > 4
					 && LeftLine[j - 1] - LeftLine[j + 1] > 4 && LeftLine[j - 1] - LeftLine[j + 2] > 4
					 && LeftLine[j - 1] - LeftLine[j + 3] > 4 && LeftLine[j - 2] - LeftLine[j + 1] > 4
					 && LeftLine[j - 2] - LeftLine[j + 2] > 4 && LeftLine[j - 2] - LeftLine[j + 3] > 4 && LeftLine[j + 1] == 0
					 && LeftLine[j + 2] == 0 && LeftLine[j + 3] == 0)
					|| (LeftLine[j] - LeftLine[j + 2] > 4 && LeftLine[j] - LeftLine[j + 3] > 4 && LeftLine[j] - LeftLine[j + 4] > 4
						&& LeftLine[j - 1] - LeftLine[j + 1] > 4 && LeftLine[j - 1] - LeftLine[j + 2] > 4
						&& LeftLine[j - 1] - LeftLine[j + 3] > 4 && LeftLine[j - 2] - LeftLine[j + 1] > 4
						&& LeftLine[j - 2] - LeftLine[j + 2] > 4 && LeftLine[j - 2] - LeftLine[j + 3] > 4 && LeftLine[j + 2] == 0
						&& LeftLine[j + 3] == 0 && LeftLine[j + 4] == 0)) {
					Circle_flag = 9;
					OutHopRow = OutHopRow1 = j - 2;
				}
			}
...
uint8 Judge_Round1(void)
{
	...
	if (LeftFlag == 1 && RightFlag == 0) {
		...
		for (p = ROW - 7; p > 20; p--) {
			if (p <= EndRow)
				return 0;
			if (((RightLine[p] - RightLine[p - 1] < -3 && RightLine[p] - RightLine[p - 2] < -9
				  && RightLine[p] - RightLine[p - 3] < -12 && RightLine[p] - RightLine[p - 4] < -16)
				 || (p > 40 && RightLine[p + 1] < 145 && RightLine[p] < 145 && RightLine[p - 1] == 151 && RightLine[p - 2] == 151
					 && RightLine[p - 3] == 151)
				 || (p > 22 && RightLine[p + 1] < 145 && RightLine[p] < 145 && RightLine[p - 1] == 151 && RightLine[p - 2] == 151
					 && RightLine[p - 3] == 151 && RightLine[p] > 127))
				&& (p < ROW - 15 && RightLine[p + 5] + 1 >= RightLine[p + 3] && RightLine[p + 6] + 1 >= RightLine[p + 3])
				&& RightLine[p + 2] != 151) {
				Cal_down_prerow = p + 1;
				break;
			}
			...
			if (RightLine[i] == 151 && RightLine[i + 1] == 151 && RightLine[i + 2] == 151) {
				for (t = i; t > 10; t--) {
					if (RightLine[t] < 151 && RightLine[t - 1] < 151 && RightLine[t - 2] < 151) {
						if (RightLine[t + 1] != 151 || RightLine[t - 10] > 145
							|| (RightLine[t] < 141 && Cal_down_prerow == 0))	// 135
						{
							ErrorFlag = 21;
							return 0;
						}
				...
				if (RightLine[i] - RightLine[i + 1] < -25 && RightLine[i] - RightLine[i + 2] < -25
					&& RightLine[i - 1] - RightLine[i + 1] < -25) {
					j			 = i;
					record_down	 = i;
					record_down1 = i;
					break;
		...
		for (; j > ValidRow + 3; j--)	 // ValidRow+3
		{
			if (j <= EndRow + 2)
				return 0;
			if (Cal_down_prerow) {
				if (Right_Slope[j + 3] < 0 && Right_Slope[j + 2] < 0 && Right_Slope[j + 1] <= 0 && Right_Slope[j] <= 0
					&& Right_Slope[j - 1] > 0 && Right_Slope[j - 2] > 0 && RightLine[j - 2] != 151 && RightLine[j - 3] != 151
					&& RightLine[j - 3] >= RightLine[j] && RightLine[j - 4] >= RightLine[j] && RightLine[j + 3] >= RightLine[j]
					&& RightLine[j + 4] >= RightLine[j] && record_down - j < 45) {
					RightFeatureRow	 = j;
				...
				// 右边界不能太靠左
				if (Right_Slope[j + 4] < 0 && Right_Slope[j + 3] < 0 && Right_Slope[j + 2] < 0 && Right_Slope[j - 2] > 0
					&& Right_Slope[j - 3] > 0 && Right_Slope[j - 4] > 0 && RightLine[j - 1] > 64 && RightLine[j] > 64
					&& RightLine[j + 1] > 64 && RightLine[j - 1] != 151 && RightLine[j - 2] != 151 && record_down - j < 45) {
					// 细找FeatureRow
		...
		if (Cal_down_prerow == 0 && record_down > 60 && RightFeatureRow > 52 && (record_down - RightFeatureRow) < 17
			&& RightLine[RightFeatureRow] > 131) {
			ErrorFlag = 101;
			return 0;
		}
		if (Cal_down_prerow == 0 && (record_down - RightFeatureRow) < 20 && record_down > 40 && RightFeatureRow > 30
			&& RightLine[RightFeatureRow] > 133) {
			ErrorFlag = 102;
			return 0;
		}
		if (Cal_down_prerow == 0 && (record_down - RightFeatureRow) < 27 && record_down > 32 && RightFeatureRow > 15
			&& RightLine[RightFeatureRow] > 133) {
			ErrorFlag = 103;
			return 0;
		}
		...
		if (Cal_down_prerow) {
			Circle_Length1 = InvSqrt((RealRightLine1[record_up] - RealRightLine1[RightFeatureRow])
										 * (RealRightLine1[record_up] - RealRightLine1[RightFeatureRow])
									 + (RealRightLine2[record_up] - RealRightLine2[RightFeatureRow])
										   * (RealRightLine2[record_up] - RealRightLine2[RightFeatureRow]));
	...
	// 左环岛
		...
		for (p = ROW - 7; p > 20; p--) {
			if (p <= EndRow)
				return 0;
			if (((LeftLine[p] - LeftLine[p - 1] > 3 && LeftLine[p] - LeftLine[p - 2] > 9 && LeftLine[p] - LeftLine[p - 3] > 12
				  && LeftLine[p] - LeftLine[p - 3] > 16)
				 || (p > 40 && LeftLine[p + 1] > 6 && LeftLine[p] > 6 && LeftLine[p - 1] == 0 && LeftLine[p - 2] == 0
					 && LeftLine[p - 3] == 0)
				 || (p > 22 && LeftLine[p + 1] > 6 && LeftLine[p] > 6 && LeftLine[p - 1] == 0 && LeftLine[p - 2] == 0
					 && LeftLine[p - 3] == 0 && LeftLine[p] < 24))
		...
		for (i = record_down_start; i > 10; i--) {
			...
			if (LeftLine[i] == 0 && LeftLine[i + 1] == 0 && LeftLine[i + 2] == 0) {
				...
				if (LeftLine[i] - LeftLine[i + 1] > 25 && LeftLine[i] - LeftLine[i + 2] > 25
					&& LeftLine[i - 1] - LeftLine[i + 1] > 25) {
					j			 = i;
					record_down	 = i;
					record_down1 = i;
					break;
		...
		for (; j > ValidRow + 3; j--) {
			if (j <= EndRow + 2)
				return 0;
			if (Cal_down_prerow) {
				if (Left_Slope[j + 3] > 0 && Left_Slope[j + 2] > 0 && (Left_Slope[j + 1] > 0 || Left_Slope[j + 1] == 0)
					&& (Left_Slope[j] > 0 || Left_Slope[j] == 0) && Left_Slope[j - 1] < 0 && Left_Slope[j - 2] < 0
					&& LeftLine[j - 2] != 0 && LeftLine[j - 3] != 0 && LeftLine[j - 3] <= LeftLine[j]
					&& LeftLine[j - 4] <= LeftLine[j] && LeftLine[j + 3] <= LeftLine[j] && LeftLine[j + 4] <= LeftLine[j]
					&& record_down - j < 45) {
					LeftFeatureRow	= j;
				...
				// 左边界不能太靠右
				if (Left_Slope[j + 4] > 0 && Left_Slope[j + 3] > 0 && Left_Slope[j + 2] > 0 && Left_Slope[j - 2] < 0
					&& Left_Slope[j - 3] < 0 && Left_Slope[j - 4] < 0 && LeftLine[j - 1] < 88 && LeftLine[j] < 88
					&& LeftLine[j + 1] < 88 && LeftLine[j - 1] != 0 && LeftLine[j - 2] != 0 && record_down - j < 45) {
					// 细找FeatureRow
					if (LeftLine[j - 1] >= LeftLine[j]) {
						if (LeftLine[j - 1] >= LeftLine[j + 1])
							a = j - 1;
						else
							a = j + 1;
					} else {
						if (LeftLine[j] >= LeftLine[j + 1])
							a = j;
						else
							a = j + 1;
					}
		...
		// 以下三段程序（ErrorFlag为101 102 103）用于防止弯入环时将下角点下端识别为record_down,主要针对大环
		if (Cal_down_prerow == 0 && record_down > 60 && LeftFeatureRow > 52 && (record_down - LeftFeatureRow) < 17
			&& LeftLine[LeftFeatureRow] < 20) {
			ErrorFlag = 101;
			return 0;
		}
		if (Cal_down_prerow == 0 && (record_down - LeftFeatureRow) < 20 && record_down > 40 && LeftFeatureRow > 30
			&& LeftLine[LeftFeatureRow] < 18) {
			ErrorFlag = 102;
			return 0;
		}
		if (Cal_down_prerow == 0 && (record_down - LeftFeatureRow) < 27 && record_down > 32 && LeftFeatureRow > 15
			&& LeftLine[LeftFeatureRow] < 18) {
			ErrorFlag = 103;
			return 0;
		}
		...
		if (Cal_down_prerow) {
			Circle_Length1 = InvSqrt((RealLeftLine1[record_up] - RealLeftLine1[LeftFeatureRow])
										 * (RealLeftLine1[record_up] - RealLeftLine1[LeftFeatureRow])
									 + (RealLeftLine2[record_up] - RealLeftLine2[LeftFeatureRow])
										   * (RealLeftLine2[record_up] - RealLeftLine2[LeftFeatureRow]));
			if (Circle_Length1 > Cir_Len / 1.0) {
				ErrorFlag = 15;
				return 0;
...
void Find_Circleborder(void)
{
	...
	if (Circle_Direction == 1) {
		for (i = COLUMN - 1; i > 5; i--) {
			if (videoData[80][i] == 1 && videoData[80][i - 1] == 1 && videoData[80][i - 2] == 1 && videoData[80][i - 3] == 1
				&& videoData[80][i - 4] == 1 && videoData[80][i - 5] == 1) {
				startcolumn = i - 3;
				break;
			}
		}
		...
		for (i = 0; i < COLUMN - 5; i++) {
			if (videoData[80][i] == 1 && videoData[80][i + 1] == 1 && videoData[80][i + 2] == 1 && videoData[80][i + 3] == 1
				&& videoData[80][i + 4] == 1 && videoData[80][i + 5] == 1)	  // ControlRow
			{
				startcolumn = i + 3;
				break;
			}
	...
	if (Circle_Direction == 1)	  // right
	{
		for (i = ROW - 4; i > endline; i--) {
			if (RightLine[i + 1] - RightLine[i] > 10 && RightLine[i + 2] - RightLine[i] > 10 && RightLine[i + 3] - RightLine[i] > 10
				&& RightLine[i + 1] - RightLine[i - 1] > 10 && RightLine[i + 2] - RightLine[i - 1] > 10
				&& RightLine[i + 3] - RightLine[i - 1] > 10 && RightLine[i + 1] - RightLine[i - 2] > 10
				&& RightLine[i + 2] - RightLine[i - 2] > 10 && RightLine[i + 3] - RightLine[i - 2] > 10
				&& RightLine[i + 1] - RightLine[i - 3] > 10 && RightLine[i + 2] - RightLine[i - 3] > 10
				&& RightLine[i + 3] - RightLine[i - 3] > 10 && RightLine[i + 1] - RightLine[i - 4] > 10
				&& RightLine[i + 2] - RightLine[i - 4] > 10 && RightLine[i + 3] - RightLine[i - 4] > 10) {
				prerow1 = i - 1;
				return (i - 1);
			}
		}
		...
		for (i = ROW - 4; i > endline; i--) {
			if (LeftLine[i] - LeftLine[i + 1] > 10 && LeftLine[i] - LeftLine[i + 2] > 10 && LeftLine[i] - LeftLine[i + 3] > 10
				&& LeftLine[i - 1] - LeftLine[i + 1] > 10 && LeftLine[i - 1] - LeftLine[i + 2] > 10
				&& LeftLine[i - 1] - LeftLine[i + 3] > 10 && LeftLine[i - 2] - LeftLine[i + 1] > 10
				&& LeftLine[i - 2] - LeftLine[i + 2] > 10 && LeftLine[i - 2] - LeftLine[i + 3] > 10
				&& LeftLine[i - 3] - LeftLine[i + 1] > 10 && LeftLine[i - 3] - LeftLine[i + 2] > 10
				&& LeftLine[i - 3] - LeftLine[i + 3] > 10 && LeftLine[i - 4] - LeftLine[i + 1] > 10
				&& LeftLine[i - 4] - LeftLine[i + 2] > 10 && LeftLine[i - 4] - LeftLine[i + 3] > 10) {
				prerow1 = i - 1;
				return (i - 1);
			}
		}
...
uint8 Find_outrow(void)	   // 可以找到：从角点在视野中出现一直到即将消失，但注意斜率为0的情况
{
	...
	for (i = ROW - 5; i > EndRow + 6; i--) {
		if (Circle_Direction == 1) {
			if ((Left_Slope[i + 2] >= 0 && Left_Slope[i + 3] > 0 && Left_Slope[i + 4] > 0 && Left_Slope[i - 3] <= 0
				 && Left_Slope[i - 4] < 0 && Left_Slope[i - 5] < 0)
				|| (Left_Slope[i + 1] > 0 && Left_Slope[i + 2] > 0 && Left_Slope[i + 3] > 0 && Left_Slope[i - 1] <= 0
					&& Left_Slope[i - 2] <= 0 && Left_Slope[i - 3] <= 0 && Left_Slope[i - 7] < 0)
				|| (Left_Slope[i + 2] > 0 && Left_Slope[i + 3] > 0 && Left_Slope[i + 4] > 0 && Left_Slope[i - 1] <= 0
					&& Left_Slope[i - 2] <= 0 && Left_Slope[i - 6] < 0 && Left_Slope[i - 7] < 0))	 // i,i-1,i-2,i+1
			{
				...
				return (uint8) Max_Row;
			}
		} else if (Circle_Direction == 2) {
			if ((Right_Slope[i + 2] <= 0 && Right_Slope[i + 3] < 0 && Right_Slope[i + 4] < 0 && Right_Slope[i - 3] >= 0
				 && Right_Slope[i - 4] > 0 && Right_Slope[i - 5] > 0)
				|| (Right_Slope[i + 1] < 0 && Right_Slope[i + 2] < 0 && Right_Slope[i + 3] < 0 && Right_Slope[i - 1] >= 0
					&& Right_Slope[i - 2] >= 0 && Right_Slope[i - 3] >= 0 && Right_Slope[i - 7] > 0)
				|| (Right_Slope[i + 2] < 0 && Right_Slope[i + 3] < 0 && Right_Slope[i + 4] < 0 && Right_Slope[i - 1] >= 0
					&& Right_Slope[i - 2] >= 0 && Right_Slope[i - 6] > 0 && Right_Slope[i - 7] > 0)) {
...
uint8  Judge_out_circle(void)	 // 可用真实赛道斜率
{
	...
	if (Circle_Direction == 1) {
		if (EndRow <= 50 && LeftLine[ControlRow + 4] > Circle_R7_outtemp && LeftLine[ROW - 1] < 100
			&& LeftLine[ROW - 2]
				   < 100)	 // 80  61  80 (EndRow<=45 && LeftLine[ControlRow+4]>65 && LeftLine[ROW-1]<80 && LeftLine[ROW-2]<85)
		{
		...
		if (EndRow <= 50 && RightLine[ControlRow + 4] < Circle_L7_outtemp && RightLine[ROW - 1] > 60
			&& RightLine[ROW - 2] > 60)	   // RightLine[ControlRow+4]<120
		{
			for (int i = ROW - 1; i > EndRow; i--) {
				if (RightLine[i] != 151) {
					rowdown = i;
...
uint8 Find_down_prerow(
	void)	 // 直道时，车的最前端在左侧路肩中间，down_prerow = 40             >60跳变值设为10 >80 找不是151的点即可 右边界一定是151
{
	...
	if (Circle_Direction == 1) {
		...
		// 右边界出现过136 137 93 138 138
		// 而误判，只有一行与其他（多）行比较是不够的，可以的话，必须多行和多行比较，利用尽可能多的信息 例如
		// i和i-1,i-2比较，且i+1和i-1,i-2比较
		if (RightFeatureRow)
			endline = RightFeatureRow + 2;

		// 等于151的条件应该放在前面，这样比较快
		for (i = 97; i >= endline; i--) {
			if ((RightLine[i] - RightLine[i - 1] < -3 && RightLine[i] - RightLine[i - 2] < -9
				 && RightLine[i + 1] - RightLine[i - 1] < -3 && RightLine[i + 1] - RightLine[i - 2] < -9
				 && RightLine[i] - RightLine[i - 3] < -15)
				|| (RightLine[i + 1] != 151 && RightLine[i] != 151 && RightLine[i - 1] == 151
					&& (i <= 38 || (RightLine[i - 2] == 151 && RightLine[i - 3] == 151)))) {
				// Circle_flag = 1;
		...
		for (i = ROW - 2; i > 97; i--) {
			if (RightLine[i + 1] < 151 && RightLine[i] < 151 && RightLine[i - 1] == 151 && RightLine[i - 2] == 151
				&& RightLine[i - 3] == 151) {
				downrow = i;
				return downrow;
			}
		...
		if (LeftFeatureRow)
			endline = LeftFeatureRow + 2;

		for (i = 97; i >= endline; i--) {
			if ((LeftLine[i] - LeftLine[i - 1] > 3 && LeftLine[i] - LeftLine[i - 2] > 9 && LeftLine[i + 1] - LeftLine[i - 1] > 3
				 && LeftLine[i + 1] - LeftLine[i - 2] > 9 && LeftLine[i] - LeftLine[i - 3] > 15)
				|| (LeftLine[i + 1] > 4 && LeftLine[i] > 4 && LeftLine[i - 1] == 0
					&& (i <= 38 || (LeftLine[i - 2] == 0 && LeftLine[i - 3] == 0)))) {
				// Circle_flag = 1;
				// Circle_Direction = 2;//left
				downrow = i + 1;
				return downrow;
...
void Along_the_other_side(void)
{
	if (down_prerow && (LeftFeatureRow || RightFeatureRow)) {
		if (Circle_Direction == 1) {
			RightLine[ControlRow - RowStep] = Revise_road(
				RightLine[down_prerow], down_prerow, RightLine[RightFeatureRow], RightFeatureRow, ControlRow - RowStep);
			RightLine[ControlRow]
				= Revise_road(RightLine[down_prerow], down_prerow, RightLine[RightFeatureRow], RightFeatureRow, ControlRow);
			RightLine[ControlRow + RowStep] = Revise_road(
				RightLine[down_prerow], down_prerow, RightLine[RightFeatureRow], RightFeatureRow, ControlRow + RowStep);
		} else {
			LeftLine[ControlRow - RowStep]
				= Revise_road(LeftLine[down_prerow], down_prerow, LeftLine[LeftFeatureRow], LeftFeatureRow, ControlRow - RowStep);
			LeftLine[ControlRow]
				= Revise_road(LeftLine[down_prerow], down_prerow, LeftLine[LeftFeatureRow], LeftFeatureRow, ControlRow);
			LeftLine[ControlRow + RowStep]
				= Revise_road(LeftLine[down_prerow], down_prerow, LeftLine[LeftFeatureRow], LeftFeatureRow, ControlRow + RowStep);
		}
...
uint8 Find_FeatureRow(void)
{
	...
	if (record_down)
		startline = record_down - 1;
	else
		startline = RowSet + Circle_Fea_Start[CircleNumber];

	if (Circle_Direction == 1) {
		if (RightFeatureRow1 - 10 > EndRow)
			endline = RightFeatureRow1 - 10;
		else
			endline = EndRow + 2;	 // EndRow
		for (i = startline; i > endline; i--) {
			// 以record_down为起点，最后判断很多0的条件就不会有Find_FeatureRow2的问题
			if (RightLine[i - 2] != 151 && RightLine[i - 3] != 151
				&& ((Right_Slope[i + 1] < 0 && (Right_Slope[i] < 0 || Right_Slope[i] == 0) && Right_Slope[i - 1] > 0
					 && Right_Slope[i - 2] > 0)
					|| (Right_Slope[i + 1] <= 0 && Right_Slope[i] <= 0 && Right_Slope[i - 1] >= 0 && Right_Slope[i - 2] >= 0
						&& Right_Slope[i - 3] >= 0 && Right_Slope[i - 4] >= 0 && Right_Slope[i - 5] >= 0 && Right_Slope[i - 6] >= 0
						&& RightLine[i] != 151 && RightLine[i - 1] != 151)
					|| (Right_Slope[i + 1] <= 0 && Right_Slope[i] <= 0 && Right_Slope[i - 4] >= 0 && Right_Slope[i - 5] >= 0
						&& Right_Slope[i - 6] >= 0 && RightLine[i] != 151 && RightLine[i - 1] != 151)
					|| (Right_Slope[i + 4] < 0 && Right_Slope[i + 3] < 0 && Right_Slope[i + 2] < 0 && Right_Slope[i - 2] > 0
						&& Right_Slope[i - 3] > 0 && Right_Slope[i - 4] > 0)
					|| (record_down > 45 && Right_Slope[i + 4] < 0 && Right_Slope[i + 3] < 0 && Right_Slope[i - 3] > 0
						&& Right_Slope[i - 4] > 0)
					|| ((record_down > 50 || record_down == 0) && Right_Slope[i + 3] == 0 && Right_Slope[i + 2] == 0
						&& Right_Slope[i + 1] == 0 && Right_Slope[i] == 0 && Right_Slope[i - 1] == 0 && Right_Slope[i - 2] == 0))) {
		...
		if (LeftFeatureRow1 - 10 > EndRow)
			endline = LeftFeatureRow1 - 10;
		else
			endline = EndRow + 2;	 // EndRow
		for (i = startline; i > endline; i--) {
			if (LeftLine[i - 3] != 0 && LeftLine[i - 2] != 0
				&& ((Left_Slope[i + 1] > 0 && (Left_Slope[i] > 0 || Left_Slope[i] == 0) && Left_Slope[i - 1] < 0
					 && Left_Slope[i - 2] < 0)
					|| (Left_Slope[i + 1] >= 0 && Left_Slope[i] >= 0 && Left_Slope[i - 1] <= 0 && Left_Slope[i - 2] <= 0
						&& Left_Slope[i - 3] <= 0 && Left_Slope[i - 4] <= 0 && Left_Slope[i - 5] <= 0 && Left_Slope[i - 6] <= 0
						&& LeftLine[i] != 0 && LeftLine[i - 1] != 0)
					|| (Left_Slope[i + 1] >= 0 && Left_Slope[i] >= 0 && Left_Slope[i - 4] <= 0 && Left_Slope[i - 5] <= 0
						&& Left_Slope[i - 6] <= 0 && LeftLine[i] != 0 && LeftLine[i - 1] != 0)
					//         ||(Left_Slope[i+1]>0&&Left_Slope[i]>=0&&Left_Slope[i-6]<0&&Left_Slope[i-7]<0)//修改处
					//         ||(Left_Slope[i+1]>0&&Left_Slope[i]>=0&&Left_Slope[i-4]<0&&Left_Slope[i-5]<0)
					|| (Left_Slope[i + 4] > 0 && Left_Slope[i + 3] > 0 && Left_Slope[i + 2] > 0 && Left_Slope[i - 2] < 0
						&& Left_Slope[i - 3] < 0 && Left_Slope[i - 4] < 0)
					|| (record_down > 45 && Left_Slope[i + 4] > 0 && Left_Slope[i + 3] > 0 && Left_Slope[i - 3] < 0
						&& Left_Slope[i - 4] < 0)
					|| ((record_down > 50 || record_down == 0) && Left_Slope[i + 3] == 0 && Left_Slope[i + 2] == 0
						&& Left_Slope[i + 1] == 0 && Left_Slope[i] == 0 && Left_Slope[i - 1] == 0 && Left_Slope[i - 2] == 0))) {
				// 细找FeatureRow
...
uint8 Find_record_down(void)	// 注意不要找成上面的,尝试修改成和FeatureRow有关
{
	...
	if (Circle_Direction == 1) {
		...
		{
			startline = (uint8) RightFeatureRow;
		} else {
		...
		for (i = startline; i < ROW - 2; i++) {
			if ((RightLine[i + 1] == 151 && RightLine[i] == 151) || (RightLine[i] >= 145 && RightLine[i + 1] < 128)
				|| (RightLine[i] - RightLine[i + 1] < -25 && RightLine[i] - RightLine[i + 2] < -25
					&& RightLine[i] - RightLine[i + 3] < -25)) {
				record_down1 = i;
				return i;
			}
		}
		...
		if (LeftFeatureRow && abs(LeftFeatureRow - LeftFeatureRow1) <= 15) {
			startline = (uint8) LeftFeatureRow;
		} else {
			startline = record_down1 - 8;
		}
		for (i = startline; i < ROW - 2; i++) {
			if ((LeftLine[i + 1] == 0 && LeftLine[i] == 0)
				|| (LeftLine[i] - LeftLine[i + 1] > 25 && LeftLine[i] - LeftLine[i + 2] > 25
					&& LeftLine[i] - LeftLine[i + 3] > 25)) {
				record_down1 = i;
				return i;
			}
		}
...
uint8 Find_record_down1(void)
{
	...
	for (i = startline; i > EndRow + 4; i--) {
		if (record_down1 > 90)
			return 90;
		if (RightLine[i + 2] == 151 && RightLine[i + 1] == 151 && RightLine[i] != 151 && RightLine[i - 1] != 151
			&& RightLine[i] > RightLine[i - 3]) {
			record_down1 = i;
			return i;
		}
	}
...
// 第0行到第99行每行校正后的纵坐标
float Y[100]
	= {221.131230504030, 209.317835549212, 198.629340615141, 188.912374348964, 180.040234667361, 171.907333459065, 164.424974374400,
	   157.518104841529, 151.122789764440, 145.184226504534, 139.655170506709, 134.494675763944, 129.667079035746, 125.141174505054,
	   120.889538481810, 116.887973264370, 113.115046329534, 109.551706316872, 106.180961280037, 102.987607735879, 99.9580013946224,
	   97.0798622775354, 94.3421083515676, 91.7347129285069, 89.2485819603306, 86.8754480656929, 84.6077786851292, 82.4386962151156,
	   80.3619083370304, 78.3716470543682, 76.4626151942582, 74.6299393283562, 72.8691282320801, 71.1760361366916, 69.5468301412481,
	   67.9779612452171, 66.4661385409707, 65.0083061711988, 63.6016227117108, 62.2434426869236, 60.9312999650211, 59.6628928135008,
	   58.4360704245845, 57.2488207445479, 56.0992594620987, 54.9856200290384, 53.9062446020442, 52.8595758078828, 51.8441492460293,
	   50.8585866527866, 49.9015896597985, 48.9719340875218, 48.0684647209166, 47.1900905204742, 46.3357802268414, 45.5045583218093,
	   44.6955013124083, 43.9077343083509, 43.1404278661547, 42.3927950760153, 41.6640888699189, 40.9535995316378, 40.2606523911636,
	   39.5846056878321, 38.9248485879156, 38.2807993438096, 37.6519035831537, 37.0376327173120, 36.4374824596085, 35.8509714445882,
	   35.2776399403581, 34.7170486467690, 34.1687775728346, 33.6324249873599, 33.1076064372687, 32.5939538285884, 32.0911145654760,
	   31.5987507430528, 31.1165383901651, 30.6441667585052, 30.1813376548142, 29.7277648131508, 29.2831733044508, 28.8472989808167,
	   28.4198879521771, 28.0006960931387, 27.5894885780172, 27.1860394421857, 26.7901311680200, 26.4015542938465, 26.0201070444146,
	   25.6455949815248, 25.2778306735416, 24.9166333826096, 24.5618287684779, 24.2132486079119, 23.8707305287439, 23.5341177576796,
	   23.2032588810373, 22.8780076176517};
double r0, roundK;
