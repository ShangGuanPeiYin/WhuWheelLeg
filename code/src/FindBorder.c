...
void FindBorder(void)
{
    ...
    for (int row = ROW-1; row >= RealEndRow; row--) // 遍历每一行
    {
        ...
        {
            if (videoData[row][col] == WHITE && videoData[row][col-1] == BLACK && videoData[row][col-2] == BLACK) // 找到黑黑白
            {
        ...
        {
            if (videoData[row][col] == WHITE && videoData[row][col+1] == BLACK && videoData[row][col+2] == BLACK) // 找到白黑黑
            {
