	...
	if (RampFlag == 0) {
		...
		if (RealEndRow == 0 /*&&MidLineSlopeVariance<0.03*/) {
			...
			for (i = ControlRow; i > RealEndRow + 8; i--) {
				...
				if (RealWidth1[i] > 60 && RealWidth1[i - 1] > RealWidth1[i] && RealWidth1[i - 2] > RealWidth1[i - 1]) {
					RampCount++;
					if (RampCount > 1) {
						RampFlag  = 1;
						// RampStrech=Stretch;
...
int OutRamp(void)
{
	if (MainCount - RampMainCount >= 0 && MainCount - RampMainCount < 50 && MainCount > 50)
		return 1;
	else
		return 0;
}
 ...
 * @brief 坡道处理，标志位置位
 */
void RampDeal(void) { RoadFlag[RAMP] = 1; }
