...
int AssertInterSectionPoint(void)
{
	...
	for (i = ROW - 1; i > ValidRow + 4; i -= div) {
		...
		if (div < 1)
			div = 1;
		if ((LeftLine[i] - LeftLine[i - div]) * leftsignflag > 0
			&& (LeftLine[i - div] - LeftLine[i - 2 * div]) * leftsignflag > 0) {
			leftsignflag  = -leftsignflag;
			leftsignflag1 = 0;
			LeftCount++;
			continue;
		} else if ((LeftLine[i] - LeftLine[i - div]) * leftsignflag1 < 0
				   && (LeftLine[i - div] - LeftLine[i - 2 * div]) * leftsignflag1 < 0) {
			leftsignflag  = 0;
	...
	for (i = ROW - 1; i > ValidRow + 4; i -= div) {
		...
		if (div < 1)
			div = 1;
		if ((RightLine[i] - RightLine[i - div]) * rightsignflag > 0
			&& (RightLine[i - div] - RightLine[i - 2 * div]) * rightsignflag > 0) {
			rightsignflag  = -rightsignflag;
			rightsignflag1 = 0;
			RightCount++;
			continue;
		} else if ((RightLine[i] - RightLine[i - div]) * rightsignflag1 < 0
				   && (RightLine[i - div] - RightLine[i - 2 * div]) * rightsignflag1 < 0) {
			rightsignflag  = 0;
...
// 小s弯识别
double nearVariance = 0, remoteVariance = 0,
	   midVariance = 0;	   // 分别代表视野中近处赛道斜率方差，中部赛道斜率方差和远处赛道斜率方差
int SnakeFlag1	   = 0;
...
int Judge_SnakeBending(void)
{
	...
	// 在小s中
		...
		// 宽度异常可认为出小s
		if (MidLineSlopeVariance < 0.32 || MidLineSlopeVariance > 0.48 || OutAssertWidth1() == 0 || RealEndRow > 5) {
			SnakeFlag = 0;
		}
		if (SnakeFlag == 0) {
			SnakeCount++;
