...
void oled_clear(void)
{
	...
	for (y = 0; 8 > y; y++) {
		...
		for (x = 0; OLED_X_MAX > x; x++) { oled_write_data(0x00); }
	}
	OLED_CS(1);
}
...
void oled_full(const uint8 color)
{
	...
	for (y = 0; 8 > y; y++) {
		...
		oled_write_command(0x01);
		oled_write_command(0x10);
		for (x = 0; OLED_X_MAX > x; x++) { oled_write_data(color); }
	}
	OLED_CS(1);
}
	...
	while ('\0' != ch[j]) {
		switch (oled_display_font) {
			case OLED_6X8_FONT: {
				...
				oled_set_coordinate((uint8) x, (uint8) y);
				for (i = 0; 6 > i; i++) { oled_write_data(ascii_font_6x8[c][i]); }
				x += 6;
				j++;
			} break;
			case OLED_8X16_FONT: {
				...
				oled_set_coordinate((uint8) x, (uint8) y);
				for (i = 0; 8 > i; i++) { oled_write_data(ascii_font_8x16[c][i]); }

				oled_set_coordinate((uint8) x, (uint8) (y + 1));
				for (i = 0; 8 > i; i++) { oled_write_data(ascii_font_8x16[c][i + 8]); }
				x += 8;
				j++;
			} break;
			case OLED_16X16_FONT: {
...
//              这个函数不可以用来直接显示总钻风的未压缩的二值化图像
//-------------------------------------------------------------------------------------------------------------------
void oled_show_binary_image(
	uint16 x, uint16 y, const uint8* image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height)
{
	// 如果程序在输出了断言信息 并且提示出错位置在这里
	// 那么一般是屏幕显示的时候超过屏幕分辨率范围了
	// 检查一下你的显示调用的函数 自己计算一下哪里超过了屏幕显示范围
...
//              如果要显示二值化图像 直接修改最后一个参数为需要的二值化阈值即可
//-------------------------------------------------------------------------------------------------------------------
void oled_show_gray_image(
	uint16 x, uint16 y, const uint8* image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height, uint8 threshold)
{
	// 如果程序在输出了断言信息 并且提示出错位置在这里
	// 那么一般是屏幕显示的时候超过屏幕分辨率范围了
	// 检查一下你的显示调用的函数 自己计算一下哪里超过了屏幕显示范围
...
void oled_show_wave(uint16 x, uint16 y, const uint16* wave, uint16 width, uint16 value_max, uint16 dis_width, uint16 dis_value_max)
{
	...
	for (y_temp = 0; y_temp < dis_value_max; y_temp += 8) {
		...
		for (x_temp = 0; x_temp < dis_width; x_temp++) { oled_write_data(0x00); }
	}
	for (i = 0; i < dis_width; i++) {
		width_index		= i * width / dis_width;
...
void OLED_Print_Img128X64(uint8 img[ROW][COL])
{
	...
	OLED_CS(0);
	for (int16 i = 0; i < 8; ++i) {
		for (int16 j = 0; j < 128; ++j) {
			oled_set_coordinate((uint8) j, (uint8) i);
			tmp = (img[(i * 8 + 0) * (ROW - 1) / 63][j * (COL - 1) / 127] << 0)		 // 锁定VSCode格式
				  | (img[(i * 8 + 1) * (ROW - 1) / 63][j * (COL - 1) / 127] << 1)	 //
				  | (img[(i * 8 + 2) * (ROW - 1) / 63][j * (COL - 1) / 127] << 2)	 //
				  | (img[(i * 8 + 3) * (ROW - 1) / 63][j * (COL - 1) / 127] << 3)	 //
				  | (img[(i * 8 + 4) * (ROW - 1) / 63][j * (COL - 1) / 127] << 4)	 //
				  | (img[(i * 8 + 5) * (ROW - 1) / 63][j * (COL - 1) / 127] << 5)	 //
				  | (img[(i * 8 + 6) * (ROW - 1) / 63][j * (COL - 1) / 127] << 6)	 //
				  | (img[(i * 8 + 7) * (ROW - 1) / 63][j * (COL - 1) / 127] << 7);
			oled_write_data(tmp);
		}
	}
	OLED_CS(1);
