...
typedef enum {
	...
	= IFX_CAN_MO_STAT_RTSEL_OFF,											   /**< \brief Receive transmit selected status flag */
 ...
 * Note: The api works only when the OCDS is enabled and in Supervisor Mode. When OCDS is disabled the OCS suspend control is
 * ineffective. \param mcan Pointer to MULTICAN module registers \param mode Module suspend mode \return None
 */
 ...
 * \return None
 */
IFX_EXTERN void IfxMultican_Node_setFastBitTiming(
	Ifx_CAN_N* hwNode, float32 moduleFreq, uint32 baudrate, uint16 samplePoint, uint16 synchJumpWidth);
 ...
 * \return None
 */
IFX_EXTERN void IfxMultican_Node_setNominalBitTiming(
	Ifx_CAN_N* hwNode, float32 moduleFreq, uint32 baudrate, uint16 samplePoint, uint16 synchJumpWidth);
...
/** \brief Disables the configuration changes for the Bit Timing Register, the Port Control Register, and the Error Counter Register
 * of a CAN Node. \param hwNode Pointer to CAN Node registers \return None
 */
IFX_INLINE void IfxMultican_Node_disableConfigurationChange(Ifx_CAN_N* hwNode);

/** \brief Enables the configuration changes for the Bit Timing Register, the Port Control Register, and the Error Counter Register
 * of a CAN Node. \param hwNode Pointer to CAN Node registers \return None
 */
 ...
 * \return None
 */
IFX_EXTERN void IfxMultican_Node_setBitTiming(
	Ifx_CAN_N* hwNode, float32 moduleFreq, uint32 baudrate, uint16 samplePoint, uint16 synchJumpWidth);
 ...
 * \return None
 */
IFX_EXTERN void IfxMultican_calcTimingFromBTR(
	float32 moduleFreq, uint32 btr, uint32* baudrate, uint16* samplePoint, uint16* synchJumpWidth);
 ...
 * \return None
 */
IFX_INLINE void IfxMultican_Message_init(
	IfxMultican_Message* msg, uint32 id, uint32 dataLow, uint32 dataHigh, IfxMultican_DataLengthCode lengthCode);
 ...
 * \return None
 */
IFX_INLINE void IfxMultican_Message_longFrameInit(IfxMultican_Message*		 msg,
												  uint32					 id,
...
IFX_INLINE void IfxMultican_MsgObj_clearFifoGatewayPointers(Ifx_CAN_MO* hwObj) { hwObj->FGPR.U = 0x0000000U; }
...
IFX_INLINE Ifx_CAN_MO* IfxMultican_MsgObj_getPointer(Ifx_CAN* mcan, IfxMultican_MsgObjId msgObjId) { return &(mcan->MO[msgObjId]); }

IFX_INLINE boolean IfxMultican_MsgObj_isExtendedFrame(Ifx_CAN_MO* hwObj) { return hwObj->AR.B.IDE != 0; }
...
IFX_INLINE void IfxMultican_MsgObj_setBitRateSwitch(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.BRS = enabled ? 1 : 0; }

IFX_INLINE void IfxMultican_MsgObj_setBottomObjectPointer(Ifx_CAN_MO* hwObj, sint32 objNumber) { hwObj->FGPR.B.BOT = objNumber; }

IFX_INLINE void IfxMultican_MsgObj_setCurrentObjectPointer(Ifx_CAN_MO* hwObj, sint32 objNumber) { hwObj->FGPR.B.CUR = objNumber; }

IFX_INLINE void IfxMultican_MsgObj_setDataCopy(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.DATC = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_MsgObj_setExtendedDataLength(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.FDF = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_MsgObj_setIdentifierCopy(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.IDC = enabled ? 1 : 0; }

IFX_INLINE void IfxMultican_MsgObj_setIdentifierExtension(Ifx_CAN_MO* hwObj, boolean extension) { hwObj->AR.B.IDE = extension; }

IFX_INLINE void IfxMultican_MsgObj_setMatchingId(Ifx_CAN_MO* hwObj, boolean matchingId) { hwObj->AMR.B.MIDE = matchingId; }
...
IFX_INLINE void IfxMultican_MsgObj_setMessageMode(Ifx_CAN_MO* hwObj, IfxMultican_MsgObjMode mode) { hwObj->FCR.B.MMC = mode; }
...
IFX_INLINE void IfxMultican_MsgObj_setOverflowInterrupt(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.OVIE = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_MsgObj_setReceiveInterrupt(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.RXIE = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_MsgObj_setRemoteMonitoring(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.RMM = enabled ? 1 : 0; }

IFX_INLINE void IfxMultican_MsgObj_setSelectObjectPointer(Ifx_CAN_MO* hwObj, sint32 objNumber) { hwObj->FGPR.B.SEL = objNumber; }

IFX_INLINE void IfxMultican_MsgObj_setSingleDataTransfer(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.SDT = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_MsgObj_setTopObjectPointer(Ifx_CAN_MO* hwObj, sint32 objNumber) { hwObj->FGPR.B.TOP = objNumber; }

IFX_INLINE void IfxMultican_MsgObj_setTransmitInterrupt(Ifx_CAN_MO* hwObj, boolean enabled) { hwObj->FCR.B.TXIE = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_Node_activate(Ifx_CAN_N* hwNode) { hwNode->CR.B.INIT = 0; }

IFX_INLINE void IfxMultican_Node_deactivate(Ifx_CAN_N* hwNode) { hwNode->CR.B.INIT = 1; }

IFX_INLINE void IfxMultican_Node_disableConfigurationChange(Ifx_CAN_N* hwNode) { hwNode->CR.B.CCE = 0U; }

IFX_INLINE void IfxMultican_Node_enableConfigurationChange(Ifx_CAN_N* hwNode) { hwNode->CR.B.CCE = 1U; }

IFX_INLINE Ifx_CAN_N* IfxMultican_Node_getPointer(Ifx_CAN* mcan, IfxMultican_NodeId node) { return &(mcan->N[node]); }

IFX_INLINE void IfxMultican_Node_resetControlRegister(Ifx_CAN_N* hwNode) { hwNode->CR.U = 0x00000041U; }

IFX_INLINE void IfxMultican_Node_resetErrorCounters(Ifx_CAN_N* hwNode) { hwNode->ECNT.U = 0x00600000U; }

IFX_INLINE void IfxMultican_Node_resetInterruptPointers(Ifx_CAN_N* hwNode) { hwNode->IPR.U = 0x00000000U; }

IFX_INLINE void IfxMultican_Node_setAlertInterrupt(Ifx_CAN_N* hwNode, boolean enabled) { hwNode->CR.B.ALIE = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_Node_setAnalyzerMode(Ifx_CAN_N* hwNode, boolean mode) { hwNode->CR.B.CALM = mode != FALSE; }

IFX_INLINE void IfxMultican_Node_setErrorWarningLevel(Ifx_CAN_N* hwNode, uint8 level) { hwNode->ECNT.B.EWRNLVL = level; }
...
IFX_INLINE void IfxMultican_Node_setLoopBackMode(Ifx_CAN_N* hwNode, boolean mode) { hwNode->PCR.B.LBM = mode != FALSE; }

IFX_INLINE void IfxMultican_Node_setReceiveErrorCounter(Ifx_CAN_N* hwNode, uint8 value) { hwNode->ECNT.B.REC = value; }
...
IFX_INLINE void IfxMultican_Node_setTransferInterrupt(Ifx_CAN_N* hwNode, boolean enabled) { hwNode->CR.B.TRIE = enabled ? 1 : 0; }
...
IFX_INLINE void IfxMultican_Node_setTransmitErrorCounter(Ifx_CAN_N* hwNode, uint8 value) { hwNode->ECNT.B.TEC = value; }

IFX_INLINE void IfxMultican_clearMessagePendingSeletor(Ifx_CAN* mcan) { mcan->MCR.B.MPSEL = 0x0U; }

IFX_INLINE void IfxMultican_clearPendingMessageNotification(Ifx_CAN* mcan, uint16 list) { mcan->MSPND[list].U = 0x0; }

IFX_INLINE void IfxMultican_disableModule(Ifx_CAN* mcan) { mcan->CLC.B.DISR = 1U; }

IFX_INLINE void IfxMultican_disableSleepMode(Ifx_CAN* mcan) { mcan->CLC.B.EDIS = 1U; }

IFX_INLINE void IfxMultican_enableModule(Ifx_CAN* mcan) { mcan->CLC.B.DISR = 0U; }

IFX_INLINE void IfxMultican_enableSleepMode(Ifx_CAN* mcan) { mcan->CLC.B.EDIS = 0U; }

IFX_INLINE uint16 IfxMultican_getFractionalDividerMode(Ifx_CAN* mcan) { return mcan->FDR.B.DM; }

IFX_INLINE uint16 IfxMultican_getFractionalDividerStepValue(Ifx_CAN* mcan) { return mcan->FDR.B.STEP; }

IFX_INLINE IfxMultican_ClockSelect IfxMultican_getInputClock(Ifx_CAN* mcan) { return (IfxMultican_ClockSelect) mcan->MCR.B.CLKSEL; }

IFX_INLINE boolean IfxMultican_isModuleEnabled(Ifx_CAN* mcan) { return mcan->CLC.B.DISS == 0; }

IFX_INLINE void IfxMultican_setFractionalDividerMode(Ifx_CAN* mcan, uint16 mode) { mcan->FDR.B.DM = mode; }

IFX_INLINE void IfxMultican_setFractionalDividerStepValue(Ifx_CAN* mcan, uint16 stepValue) { mcan->FDR.B.STEP = stepValue; }

IFX_INLINE void IfxMultican_setInputClock(Ifx_CAN* mcan, IfxMultican_ClockSelect clockSelect) { mcan->MCR.B.CLKSEL = clockSelect; }

IFX_INLINE void IfxMultican_setMessageIndexMask(Ifx_CAN* mcan, uint32 mask) { mcan->MSIMASK.U = mask; }
